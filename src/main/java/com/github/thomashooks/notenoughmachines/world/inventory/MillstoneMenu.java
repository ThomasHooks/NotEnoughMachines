...
{
    public static final Component TITLE = Component.translatable("container." + NotEnoughMachines.MOD_ID + ".millstone");
    ...
    public MillstoneMenu(int containerId, Inventory playerInventory, FriendlyByteBuf additionalData)
    {
    ...
    public MillstoneMenu(int containerId, Inventory playerInventory, BlockEntity entity, ContainerData data)
    {
        super(AllMenus.MILLSTONE.get(), containerId);
        if (entity instanceof MillstoneBlockEntity millstoneEntity)
            this.blockEntity = millstoneEntity;
        else
            throw new IllegalStateException("MillstoneMenu: Invalid block entity! Expected block entity of type MillstoneBlockEntity but received %s".formatted(BlockEntity.class.getCanonicalName()));
        ...
        millstoneEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent( itemHandler ->
        {
            //Input slot
    ...
    public ItemStack quickMoveStack(Player player, int index)
    {
        ...
        else if (index < MACHINE_INVENTORY_END_INDEX)
        {
            ...
            throw new IllegalStateException(NotEnoughMachines.MOD_ID + ":MillstoneMenu slot index is out of bounds");
    ...
    @Override
    public boolean stillValid(Player player) { return stillValid(this.levelAccess, player, AllBlocks.MILLSTONE.get()); }
    ...
    public MillstoneBlockEntity getBlockEntity() { return blockEntity; }
