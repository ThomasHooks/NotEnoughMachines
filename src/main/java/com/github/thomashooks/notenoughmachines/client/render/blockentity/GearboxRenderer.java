...
public class GearboxRenderer implements BlockEntityRenderer<GearboxBlockEntity>
{
    ...
    @Override
    public void render(GearboxBlockEntity entity, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay)
    {
        poseStack.pushPose();
        ...
        switch (entity.getBlockState().getValue(MechanicalShaftBlock.AXIS))
        {
            ...
            case Z -> poseStack.mulPose(new Quaternionf().rotateLocalY(Mth.HALF_PI));
        }
        poseStack.translate(-0.5D, -0.5D, -0.5D);
        ...
        poseStack.translate(0.5D, 0.5D, 0.5D);
        //Have to rotate back to render the other axle
        poseStack.mulPose(new Quaternionf().rotateLocalY(angle));
