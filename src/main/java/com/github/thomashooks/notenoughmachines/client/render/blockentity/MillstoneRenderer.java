...
{
    public static final Material RESOURCE_LOCATION = new Material(TextureAtlas.LOCATION_BLOCKS, new ResourceLocation(NotEnoughMachines.MOD_ID + ":block/machine/millstone_runner_stone"));
    private final ModelPart axleBody;
    private final ModelPart runnerStoneBody;
    ...
    public MillstoneRenderer(BlockEntityRendererProvider.Context context)
    {
        ...
        ModelPart waterWheelModelPart = context.bakeLayer(MillstoneModelLayer.LOCATION);
        this.runnerStoneBody = waterWheelModelPart.getChild("runner_stone");
    }

    @Override
    public void render(MillstoneBlockEntity entity, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int combinedLight, int combinedOverlay)
    {
        poseStack.pushPose();
        ...
        VertexConsumer runnerStoneVertexConsumer = RESOURCE_LOCATION.buffer(bufferSource, RenderType::entityCutout);
        this.runnerStoneBody.render(poseStack, runnerStoneVertexConsumer, combinedLight, combinedOverlay);
