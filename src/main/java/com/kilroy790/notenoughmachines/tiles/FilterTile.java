...
public class FilterTile extends ItemConduitTile implements INamedContainerProvider 
{
	...
	private LazyOptional<CombinedInvWrapper> combinedHandler = LazyOptional.of(() -> new CombinedInvWrapper(this.inventory.get(BUFFER), this.inventory.get(RED_FILTER), this.inventory.get(GREEN_FILTER), this.inventory.get(BLUE_FILTER), this.inventory.get(YELLOW_FILTER)));
