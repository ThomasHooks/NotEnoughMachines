...
public class FilterContainer extends NEMBaseContainer<FilterTile> {
	...
	public ItemStack transferStackInSlot(PlayerEntity player, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (index == BUFFERSLOT) {
				if(!this.mergeItemStack(itemstack1, PLAYER_INV_START, PLAYER_INV_END, false)) return ItemStack.EMPTY;
			}
			...
			if (index < PLAYER_INV_END && index >= PLAYER_INV_START) {
				if(canMergeSlot(itemstack1, getSlot(BUFFERSLOT))) {
					if (!this.mergeItemStack(itemstack1, BUFFERSLOT, FILTERSLOT, false)) return ItemStack.EMPTY; 
				}
				else {
					if(index < PLAYER_INV_END - 9) {
						if(!this.mergeItemStack(itemstack1, PLAYER_INV_END - 9, PLAYER_INV_END, true)) return ItemStack.EMPTY;
					}
					else {
						if(!this.mergeItemStack(itemstack1, PLAYER_INV_START, PLAYER_INV_END - 9, false)) return ItemStack.EMPTY;
					}
				}
			}
