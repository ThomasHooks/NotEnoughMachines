...
{	
	...
	{
		//NotEnoughMachines.LOGGER.info("Removing Power Network space for world " + worldIn.getDimensionType().getType().getRegistryName());
	...
	public void joinPowerNetwork(MechanicalTile tile) 
	{
		...
		if (networkID != 0) 
			addBackToPowerNetwork(tile, worldPowerNetworks);
		
		else 
			networkID = createOrAddToPowerNetwork(tile, worldPowerNetworks);
	...
	{
		if (subnetwork.contains(tileCurrent)) 
			return;
	...
	{	
		...
		if (tile.getNetworkID() > 0) 
			universePowerNetworks.get(tile.getWorld()).get(tile.getNetworkID()).update();
	}
}
